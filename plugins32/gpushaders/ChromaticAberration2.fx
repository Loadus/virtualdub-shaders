...
/*
	...
	smooth Chromatic Aberration effect by Martins Upitis	

	Ported to VirtualDub's vdshader by Jukka Korhonen (loadus.deviantart.com)
...
float2 Random (float2 input) {
	...
	float noise1 = (fract(sin(dot(input, float2(12.9898, 78.233))) * 43758.5453));
	float noise2 = (fract(sin(dot(input, float2(12.9898, 78.233) * 2.0)) * 43758.5453));

	return clamp(float2(noise1, noise2), 0.0, 1.0);
}

float3 Blur (float2 coords) {

	float2 noise = Random(TEXCOORD0.xy)
}
