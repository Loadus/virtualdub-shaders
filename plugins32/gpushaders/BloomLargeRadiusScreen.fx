...
float4 BloomPS (float2 uv : TEXCOORD0) : COLOR0 {
	...
	float sharpness;
	float sigmoidal_base; 
	float sigmoidal_range;
	float mid_point; 
	...
	// D = 1.0 / (1.0 + exp(-1.0 * (sigmoidal_base +  sharpness * sigmoidal_range) * (D - mid_point)));


   		B = pow(B, gamma * 0.5);
   		//D = pow(D, gamma); 
	
		B = lerp(0.0f, B, flatten);

   // float4 C = (1 - D) * (D * B) + D * (1 - (1 - D) * (1 - B));
   // float4 C = (1 - A) * (A * B) + A * (1 - (1 - A) * (1 - B));
   float4 C = 1 - (1 - D) * (1 - B);
   ...
   // float4 C;
   		...
   		// }
			...
			// D = dot(float3(0.333, 0.333, 0.333), C.rgb);
	...
	sharpness = s_sharpness_r;
	sigmoidal_base = 0; 
	sigmoidal_range = sigmoid_r;
	mid_point = mid_r;

	C.r = 1.0 / (1.0 + exp(-1.0 * (sigmoidal_base +  sharpness * sigmoidal_range) * (C.r - mid_point)));

	sharpness = s_sharpness_g;
	sigmoidal_base = 0; 
	sigmoidal_range = sigmoid_g;
	mid_point = mid_g;

	C.g = 1.0 / (1.0 + exp(-1.0 * (sigmoidal_base +  sharpness * sigmoidal_range) * (C.g - mid_point)));

	sharpness = s_sharpness_b;
	sigmoidal_base = 0; 
	sigmoidal_range = sigmoid_b;
	mid_point = mid_b;

	C.b = 1.0 / (1.0 + exp(-1.0 * (sigmoidal_base +  sharpness * sigmoidal_range) * (C.b - mid_point)));
